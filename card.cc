...
/**
 * @brief Card::stackCard
 * Kortin pinoamiseen tarkoitettu metodi
 * @param card, kortti joka laitettaan päälle
 * @param NoOffset, lippu osoittamaan tuleeko kortti pinoon limittäin
 */
void Card::stackCard(Card *card, bool NoOffset)
{
    card->setParent(this);

    // Jos kortti laitetaa suitSlottiin niin korteille ei offsettia
    if(NoOffset)
        card->move(0, 0);
    else
...
void Card::setupFront()
{
    ...
    // Kortin alareunaan kortin maa ja numero
    painter.drawPixmap(frontPixmap.width()/2-suitPixmap_.width(), CARD_HEIGHT - 30,
                       suitPixmap_, 0, 0, 0, 0);
    painter.drawText(frontPixmap.width()/2, CARD_HEIGHT - 33, suitPixmap_.width() + 5,
                     suitPixmap_.height() + 5, Qt::AlignCenter, QString::number(value_));
