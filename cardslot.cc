...
CardSlot::CardSlot(CheckFunction function, QWidget *parent, bool suitSlot):
    QFrame(parent), topCard_(nullptr), checkFunction_(function), suitSlot_(suitSlot)
{
...
void CardSlot::setupNewCards(std::list<Card *> &newCards)
{
    ...
    // Jos lisätään useampia kortteja tyhjään paikkaan
        ...
        for (auto card: newCards){
            topCard_->stackCard(card, suitSlot_);
            topCard_  = card;
            card->show();
        }
    }
...
/**
 * @brief CardSlot::checkWin
 * Tarkistaa onko kyseessä suitSlot ja onko kortti maan viimeinen eli 13
 * @param card, tarkistettava kortti
 */
void CardSlot::checkWin(Card* card)
{
    if(suitSlot_)
    {
        // Hankkii kortin arvon
        std::string cardData = card->getCardData();
        int value = stoi(cardData.substr(0,cardData.find_first_of(",")));
        if(value == 13)
        {
            // Jos kortti oli maan viimeinen lähetetään signaali ja suljetaan slot
            emit slotStatus(1);
            setDisabled(true);
        }
    }
